...
function Home() {
  ...
  const [cartList, setCartList] = useState(
    localStorage.getItem("CartFight") !== null
      ? JSON.parse(localStorage.getItem("CartFight"))
      : []
  );

  async function gatherPokemonData() {
    const typeInfo = await fetch("https://pokeapi.co/api/v2/type/2");
  ...
  const checkout = () => {
    localStorage.removeItem("CartFight");
  };
